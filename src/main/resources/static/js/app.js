...
const app = createApp({
  setup() {
    ...
    const prepareChartData = (stockIndex) => {
      ...
      // 提取均线数据（如果存在）
      let ma = [];
      if (currentDataType.value === 'halfYearLine' && sortedData[0].length > 10) {
        ma = sortedData.map(item => item[10]); // ma120
      } else if (currentDataType.value === 'yearLine' && sortedData[0].length > 10) {
        ma = sortedData.map(item => item[10]); // ma250
      }
    ...
    const renderCharts = () => {
      ...
      const option = {
        ...
        legend: {
          data: currentDataType.value === 'halfYearLine' || currentDataType.value === 'yearLine'
            ? ['股价', '成交量', currentDataType.value === 'halfYearLine' ? '半年线' : '年线']
            : ['股价', '成交量'],
          top: 30
        },
        dataZoom: [
